...
async function login (req, res) {
  try {
    ...
    bcrypt.compare(req.body.password, user.password, (err, result) => {
      ...
      res.status(200).json({ _id:user._id, email: user.email, name: user.name, description:user.description, descriptionShort:user.descriptionShort, img:user.img, token })
    })
  } catch (error) {
